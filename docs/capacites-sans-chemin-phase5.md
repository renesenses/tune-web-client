...
> ✅ Porté par `feat/v2-porte-appliance` (visible seulement si `config.appliance`, niveau Expert pour le stockage).

- `applianceInstallStatus` — `GET /appliance/install-to-disk/status` — ✅ **porté** : Réglages › Système › Emplacement des données (`ApplianceStockageV2`, sondage après installation)
- `applianceInstallToDisk` — `POST /appliance/install-to-disk` — ✅ **porté** : Réglages › Système › Emplacement des données (`ApplianceStockageV2`, « Installer Tune OS ici », saisie EFFACER)
- `applianceMountVolume` — `POST /appliance/storage/mount` — ✅ **porté** : Réglages › Système › Emplacement des données (`ApplianceStockageV2`, « Utiliser comme dossier musique »)
- `applianceRelocateData` — `POST /appliance/data/relocate` — ✅ **porté** : Réglages › Système › Emplacement des données (`ApplianceStockageV2`, « Déplacer ici », confirmation danger)
- `applianceShutdown` — `POST /appliance/shutdown` — ✅ **porté** : Réglages › Système › Santé du serveur (`ApplianceEteindreV2`, « Éteindre », confirmation danger)
- `getApplianceDataStatus` — `GET /appliance/data/status` — ✅ **porté** : Réglages › Système › Emplacement des données (`ApplianceStockageV2`)
- `getApplianceStorage` — `GET /appliance/storage` — ✅ **porté** : Réglages › Système › Emplacement des données (`ApplianceStockageV2`)
