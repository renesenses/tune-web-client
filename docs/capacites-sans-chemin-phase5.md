...
- `getAirplayPairStatus` — `GET /outputs/{}/airplay/pair-status` — ✅ **porté** : Zones v2, vue Liste, bouton cadenas d'une zone AirPlay → panneau d'appairage (`AppairageAirplayV2.svelte`)
- `getZoneDevicePresets` — `GET /zones/{}/device-presets` — ✅ **porté** : Réglages v2 → Appareils → Réglages par zone, proposition sur la carte de chaque zone (`PrereglageAppareilV2.svelte`) ; « Appliquer » envoie `applyZoneDevicePreset` (le PATCH listé plus bas parmi les « orphelines de nom »)
- `startAirplayPairing` — `POST /outputs/{}/airplay/pair-start` — ✅ **porté** : même panneau, « Démarrer l'appairage »
- `submitAirplayPairPin` — `POST /outputs/{}/airplay/pair-pin` — ✅ **porté** : même panneau, saisie du code affiché par le récepteur
- `uploadAudioFile` — `POST /zones/upload` — ✅ **porté** : File d'attente v2, bouton « Ajouter un fichier » et dépôt de fichiers sur la file (`AjoutFichierFileV2.svelte`, `lib/ajoutFichiersAudio.ts`)

Témoin : `src/lib/__tests__/porteV2Sorties.test.ts`.
...
- applyZoneDevicePreset — PATCH /zones/{} — ≡ updateZoneWavMode — ✅ **porté** avec `getZoneDevicePresets` (Réglages v2 → Appareils → Réglages par zone, « Appliquer »)
