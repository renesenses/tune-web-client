...
- `createEqPreset` — `POST /eq/presets` — ✅ **porté** : Égaliseur v2 (`EqualizerV2`), bloc « Mes presets » (greffon installé), « Enregistrer »
- `deleteEqPreset` — `DELETE /eq/presets/{}` — ✅ **porté** : Égaliseur v2 (`EqualizerV2`), bloc « Mes presets » (greffon installé), « × » après confirmation danger
- `listEqPresets` — `GET /eq/presets` — ✅ **porté** : Égaliseur v2 (`EqualizerV2`), bloc « Mes presets » (greffon installé)
