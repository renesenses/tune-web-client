...
## Hors ligne (5) — ✅ PORTÉ (feat/v2-porte-hors-ligne)

Désormais atteint par **Réglages → Bibliothèque → Écoute hors-ligne**
(`src/components/v2/HorsLigneV2.svelte`, niveau Intermédiaire). Les cinq routes
existent sur le serveur (`tune-server/src/routes/offline.rs`). Témoins :
`src/lib/__tests__/horsLigneV2.test.ts`.

- `clearOffline` — `POST /offline/clear` → HorsLigneV2, « Tout supprimer » (confirmation `danger`)
- `getOfflineDownloads` — `GET /offline/downloads` → HorsLigneV2, liste des pistes
- `getOfflineStatus` — `GET /offline/status` → HorsLigneV2, compteurs
- `removeOfflineDownload` — `DELETE /offline/downloads/{}` → HorsLigneV2, « Supprimer » par piste
- `syncOffline` — `POST /offline/sync` → HorsLigneV2, « Synchroniser »

⚠️ Aucun écran (ni ancien ni v2) n'appelle `downloadForOffline` : le client web
ne remplit pas ce cache, il le gère seulement.
