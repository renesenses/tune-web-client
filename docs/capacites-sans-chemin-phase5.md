...
- `getMarketplaceCatalog` — `GET /marketplace/plugins` — ✅ **porté** : Réglages › Extensions (`PluginsV2`), onglet « Catalogue » (fusion `lib/catalogueGreffons.ts`)
- `uninstallPlugin` — `DELETE /plugins/{}` — ✅ **porté** : Réglages › Extensions (`PluginsV2`), « Désinstaller » d'un greffon non wasm, après confirmation danger
- `updatePlugin` — `POST /plugins/{}/update` — ✅ **porté** : Réglages › Extensions (`PluginsV2`), « Mettre à jour » quand `update_available`
