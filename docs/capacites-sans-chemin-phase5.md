...
- `ignoreDevice` — `POST /devices/{}/ignore` — ✅ **porté** : Réglages v2 → Audio → Appareils réseau, la croix de chaque appareil (`SettingsV2.svelte`, `ignorerAppareil`) ; elle appelait jusque-là `DELETE /devices/{id}`, l'oubli en mémoire que #1280 avait remplacé
- `listIgnoredDevices` — `GET /devices/ignored` — ✅ **porté** : Réglages v2 → Audio → Appareils ignorés (`AppareilsIgnoresV2.svelte`)
- `unignoreDevice` — `DELETE /devices/{}/ignore` — ✅ **porté** : même section, bouton « Ne plus ignorer » sur chaque ligne

Témoin : `src/lib/__tests__/porteV2AppareilsIgnores.test.ts` (monte Réglages v2, clique la croix puis « Ne plus ignorer »).
