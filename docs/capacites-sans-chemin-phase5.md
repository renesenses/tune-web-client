...
- `discoverSmbShares` — `GET /network/shares` — ✅ **porté** : Réglages v2 → Bibliothèque → Partages réseau, « Scanner le réseau » (`PartagesReseauV2.svelte`)
- `getNetworkDiagnostics` — `GET /system/diagnostics/network` — ⛔ **non porté, capacité morte** : l'écran `DiagnosticsView` lit `multicast_ssdp`, `port_8888`, `internet`, `dns_resolution`, `renderers` ; le serveur Rust (`diagnostics_network`, `system/diagnostics.rs`) rend `discovered_devices`, `registered_outputs`, `slimproto`, `lms_cli`, `slimproto_udp`, `ssdp`, `devices` — aucun champ commun, et `git log -S multicast_ssdp` ne trouve aucune version du serveur Rust qui les ait jamais rendus. L'ancienne carte affichait donc trois ❌ en permanence. Une vraie carte réseau (SSDP, SlimProto) est à écrire sur le contrat réel, pas à porter.
- `listSmbMounts` — `GET /network/smb/mounts` — ✅ **porté** : même section, état RÉEL de chaque montage (#2069), cause d'échec et badge SMB 1.0
- `mountSmbShare` — `POST /network/smb/mount` — ✅ **porté** : même section, « Monter le partage », puis « Ajouter à la bibliothèque » et « Scanner la bibliothèque »
- `scanHost` — `GET /network/scan-host` — ✅ **porté** : même section, partages d'un hôte découvert (#3637) ou d'une adresse saisie (#1846)
- `testSmbConnection` — `POST /network/smb/mount` (`dry_run`) — ✅ **porté** : même section, « Tester la connexion »

Témoin : `src/lib/__tests__/porteV2PartagesReseau.test.ts` (monte l'assistant et Réglages v2, clique de la découverte au scan).
