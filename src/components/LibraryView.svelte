...
import CollapsibleSection from './CollapsibleSection.svelte';
  ...
  // L'image communautaire (image_url) peut être une URL 404 : on retombe alors
  // sur les initiales au lieu de laisser un trou (bug forum images artiste).
  let artistImageFailed = $state(false);
      ...
      <div class="artist-detail-header">
        <div class="artist-detail-avatar">
          {#if $selectedArtist.image_path}
            <AlbumArt coverPath={$selectedArtist.image_path} size={160} alt={$selectedArtist.name} round fallbackText={initials($selectedArtist.name)} />
          {:else if artistMetadata?.image_url && !artistImageFailed}
            <img class="artist-detail-img" src={artistMetadata.image_url} alt={$selectedArtist.name} loading="lazy" onerror={() => artistImageFailed = true} />
          {:else}
          ...
          <div class="artist-card" onclick={() => selectArtistDetail(artist)}>
            <div class="artist-card-avatar">
              {#if artist.image_path}
                <AlbumArt coverPath={artist.image_path} size={100} alt={artist.name} round fallbackText={initials(artist.name)} />
              {:else}
