<script lang="ts">
  ...
  async function importer(ev: Event) {
    ...
    importEnCours = true;
    try {
      // Une playlist LINN (`.dpl`, Kazoo / Linn Konfig) a sa propre route, qui
      // rapproche les entrées de la bibliothèque et crée la playlist d'un
      // coup — portée de l'ancien écran Réglages avant la phase 5 (web#1257).
      if (f.name.toLowerCase().endsWith('.dpl')) {
        const l = await api.importLinnPlaylist(f);
        notifications.success(`${$t('v2.pl.importDone' as any)} — ${l.name} — ${l.matched}/${l.total_entries}`);
        load();
        return;
      }
