<script lang="ts">
   ...
   *
   * Phase 5 (web#1257) : l'assistant n'existait que dans l'ancien
   * `SettingsView`, que #1257 a retiré. Le nouveau client affichait à sa
   * place « l'import reste dans le client actuel » — une porte fermée.
   * Écran repris de #1276 (import seul, sans ce que cette PR dupliquait).
   *
   * Il marche côté serveur (tune-server-rust #3914, R4/R5) : le fichier
   * téléversé est lu (CSV Roon, XML Plex), `?preview=true` CALCULE le rapport
   * sans rien écrire, et l'import réel rend `202 {task_id}` puis travaille
   * derrière ; son rapport se lit sur `GET /system/import/status/{id}`. D'où
   * trois temps : aperçu, confirmation, suivi jusqu'au rapport final.
   *
   * 🔴 Les listes M3U/PLS ne sont PAS proposées : `import_playlists_file` est
   * un bouchon serveur (« not yet implemented »), qui répond « accepted »
   * sans rien importer. Les listes Linn `.dpl` passent par `importLinnPlaylist`.
   */
  ...
  function appel(s: Source, f: File, apercu: boolean): Promise<any> {
    return s === 'roon' ? api.importRoon(f, apercu) : api.importPlex(f, apercu);
  }

  /** Suivi de la tâche : 1,5 s entre deux lectures, 15 min au plus. */
  const PAS_SUIVI_MS = 1500;
  const BUDGET_SUIVI_MS = 15 * 60 * 1000;
  let suivi = $state(false);
  let enFond = $state(false);
  let detruit = false;
  $effect(() => () => { detruit = true; });

  async function suivre(taskId: string) {
    suivi = true;
    const depart = Date.now();
    try {
      while (!detruit) {
        const e = await api.getImportStatus(taskId);
        if (e?.status && e.status !== 'running') {
          if (e.status === 'unknown') { enFond = true; return; }
          rapport = e;
          if ((e.errors ?? 0) > 0) {
            erreur = get(t)('v2.import.rowErrors' as any).replace('{count}', String(e.errors));
          }
          return;
        }
        if (Date.now() - depart > BUDGET_SUIVI_MS) { enFond = true; return; }
        await new Promise((r) => setTimeout(r, PAS_SUIVI_MS));
      }
    } catch {
      // Suivi perdu (réseau) : l'import continue côté serveur, on le dit.
      enFond = true;
    } finally {
      suivi = false;
    }
  ...
  async function confirmer() {
    ...
    erreur = null;
    try {
      const r = await appel(source, fichier, false);
      etape = 'fait';
      rapport = null;
      enFond = false;
      if (r?.task_id) await suivre(r.task_id);
      else rapport = r;
    } catch (err: any) {
...
<div class="imp">
  ...
  {:else}
    {#if suivi}
      <p class="hint" role="status">{$t('v2.import.running' as any)}</p>
    {:else}
    ...
    {:else if rapport}
      <div class="chiffres">
        <div class="chiffre"><b>{rapport.total_rows ?? 0}</b><span>{$t('import.totalRows' as any)}</span></div>
        <div class="chiffre ok"><b>{rapport.matched ?? 0}</b><span>{$t('import.matched' as any)}</span></div>
        <div class="chiffre manque"><b>{rapport.unmatched ?? 0}</b><span>{$t('import.unmatched' as any)}</span></div>
