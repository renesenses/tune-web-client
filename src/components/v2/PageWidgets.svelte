<script lang="ts">
   ...
   * l'utilisateur : ajouter, retirer, réordonner. Puis, le même jour : « je
   * voudrais que les écrans éditorial Qobuz et Tidal soient paramétrables avec
   * des widgets Qobuz et Tidal ».
   *
   * D'où le CATALOGUE en paramètre. Deux pages qui se composent de la même
   * façon ne valent pas deux composants : l'accueil passe le registre général,
   * un écran de service passe le sien, et tout le reste — chargement paresseux,
   * édition, persistance par profil — ne connaît ni l'un ni l'autre.
   *
   * ## Ce que ce composant sait
   *
   * Presque rien. Il lit une liste d'identifiants de widgets, demande au
   * catalogue de charger chacun, et rend une bande horizontale. Aucun widget
   * n'est écrit ici — en ajouter un se fait dans un registre, sans toucher à
   * cet écran.
   *
  ...
  interface Props {
    ...
    catalogue?: Widget[];
    /** Disposition d'un profil qui n'a rien encore choisi. */
    dispositionDefaut?: string[];
    /**
     * Clé sous laquelle la disposition est rangée dans les préférences DU
     * PROFIL. Une par page : sans cela, composer l'écran Qobuz déferait
     * l'accueil.
     */
    cle?: string;
    cleEyebrow?: string;
