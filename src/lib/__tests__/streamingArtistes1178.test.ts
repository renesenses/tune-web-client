...
import { artisteDeService } from '../ouvrirArtisteDepuis';
...
describe('Streaming : ouvrir un artiste puis revenir', () => {
  ...
  // 🔴 Contrat ADAPTÉ à la promotion du 20/09 (lot `batch/jp-p1-web-routing-20260918`
  // vers `main`). Écrit le 18/09, ce témoin exigeait qu'une identité incomplète
  // reste un `<span>` inerte. Depuis, #1194 (FabienM, fil 1839, point 9) a rendu
  // TOUTES les vignettes cliquables : un artiste réduit à son NOM part chercher
  // la fiche locale par nom exact (`artisteDeService` rend `{ name }`, contrat
  // couvert par `artisteFavoriDeService1194.test.ts`).
  //
  // Ce qui reste de #1178, et que ce témoin garde : une identité incomplète ne
  // fabrique JAMAIS de route de SERVICE — ni fiche, ni requête `/streaming/…/artists/`.
  it('les identités incomplètes ne fabriquent aucune route de service', async () => {
    artistes = [
    ...
    // Aucune de ces vignettes ne porte d'identité de service exploitable.
    for (const ar of artistes) {
      expect(artisteDeService(ar, 'qobuz')?.source_id, `${ar.name} : route de service fabriquée`).toBeUndefined();
    }
    ...
    (cartes[0].querySelector('button.an') as HTMLButtonElement).click();
    flushSync();
    // #1194 : le nom seul retombe sur la bibliothèque, jamais sur une fiche de service.
    await vi.waitFor(() => expect(get(activeView)).toBe('library'));
