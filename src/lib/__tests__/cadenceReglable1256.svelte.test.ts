...
beforeEach(() => {
  fileRaf = [];
  // 🔴 APRÈS `performance.now()` réel : `AudioVisualizer.startAnimation` pose
  // `lastFrame = performance.now()` et compare l'horodatage rAF à cette
  // valeur. Une horloge de test partie de 1000 rendrait des écarts NÉGATIFS,
  // le composant ne dessinerait jamais, et les trois crans rendraient 0 —
  // un faux vert « le cran minimal dessine peu » compris.
