 ...
 *
 * `is_compilation` (serveur #4427, livré en 0.9.155) : absent veut dire « je
 * n'y touche pas », jamais « faux » — on change le genre d'une sélection sans
 * lui reprendre son drapeau. Le serveur pose en même temps le marqueur
 * d'édition manuelle, ce qui empêche le scan de revenir sur la décision.
 *
 * ⚠️ Il n'existe PAS de route `/library/albums/compilation` : une seconde
 * implémentation de #4427 en proposait une, elle a été fermée sans être
 * fusionnée. Le drapeau passe par ICI. Voir la garde de
 * `tagCompilation1172.test.ts`.
 */
