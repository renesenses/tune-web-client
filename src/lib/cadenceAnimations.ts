 ...
 *
 * `CRAN_CADENCE_DEFAUT` vaut `fluide`, et `fluide` rend EXACTEMENT les deux
 * constantes d'aujourd'hui — 1000/30 ms pour le crête-mètre (`cadenceCreteMetre`),
 * 33 ms entiers pour le visualiseur (`AudioVisualizer.FRAME_INTERVAL`). Les deux
 * arrondis diffèrent, et cette différence est CONSERVÉE : personne ne doit voir
 * son affichage changer sans l'avoir demandé, pas même d'une image.
 *
...
/**
 * Intervalle du VISUALISEUR (spectre et forme d'onde), en millisecondes.
 *
 * 🔴 Arrondi à l'entier : au cran `fluide` il vaut `Math.round(1000 / 30) = 33`,
 * la valeur exacte du `FRAME_INTERVAL = 33` livré dans `AudioVisualizer.svelte`.
 * Les deux composants n'ont jamais eu le même arrondi ; ce n'est pas élégant,
 * mais l'uniformiser au passage changerait le défaut de l'un des deux.
 *
 * (Ce désaccord d'un tiers de milliseconde entre deux cadenceurs est exactement
 * ce qui avait fait disparaître une image sur deux dans la variante C de #1499.
 * Ici il n'y a qu'UN cadenceur par boucle, et il n'y en aura pas deux.)
 */
export function intervalleVisualiseurMs(cran: CranCadence): number {
  return Math.round(1000 / CADENCE_HZ[cran]);
}
